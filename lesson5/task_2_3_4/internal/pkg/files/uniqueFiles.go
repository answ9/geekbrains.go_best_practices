...
func (uf *UniqueFiles) Find(searchPath string, workers int) int {
	...
	go func(dir string, files chan<- File) {
		err := filepath.Walk(dir, func(path string, info os.FileInfo, err error) error { // linter: Error return value of `filepath.Walk` is not checked (errcheck)
			if err != nil {
				return fmt.Errorf("error during filepath walk %s", fmt.Sprintf("%s/%s", dir, path)) // ST1005: error strings should not be capitalized (stylecheck)
			}
...
func (uf *UniqueFiles) Print(searchPath string) {
	...
	counter := 1
	for k, _ := range uf.Map { // linter: File is not `gofmt`-ed with `-s` (gofmt) Так и не понял, что он от меня хочет
		var txt string
