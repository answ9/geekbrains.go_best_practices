...
// linter: `Crawler` is unused (deadcode)
...
// linter: type `reqWithDelay` is unused (unused)
//type reqWithDelay struct {
//	delay time.Duration
//	req   Requester
//}

// linter: cmd/main.go:99:6: `NewRequestWithDelay` is unused (deadcode)
//func NewRequestWithDelay(delay time.Duration, req Requester) *reqWithDelay {
//	return &reqWithDelay{delay: delay, req: req}
//}

// linter: undeclared name: `reqWithDelay` (typecheck)
...
func main() {
	...
	//var r Requester //linter: S1021: should merge variable declaration with assignment on next line (gosimple)
	//r = NewRequester(time.Minute)
	var r = NewRequester(time.Minute)
	...
	go processResult(crawler.GetResultChan(), cnfg, cancel)
	for {
		select {
		case s := <-chSig:
			switch s {
	...
	//cancel() // linter: unreachable: unreachable code (govet)
}

func processResult(in <-chan CrawlResult, cnfg *config.AppConfig, cancel context.CancelFunc) {
	ctx, cancelFunc := context.WithTimeout(context.Background(), time.Duration(cnfg.TimeOut)*time.Second) // linter: SA4009: argument ctx is overwritten before first use (staticcheck)
